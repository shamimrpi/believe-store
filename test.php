							...
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-sm-8">
										...
										<v-select v-bind:options="categories" v-model="selectedCategory" label="ProductCategory_Name" @input="onChangeCategory"></v-select>
									</div>
									<div class="col-sm-1" style="padding: 0;">
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"> Pro. Name </label>
									<div class="col-sm-9">
										<input type="text" id="Product_Name" ref="Product_Name"  name="Product_Name" class="form-control" v-model="selectedProduct.Product_Name" required/>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-4">
										...
										<v-select v-bind:options="colors" v-model="selectedColor" label="color_name" v-if="colors.length > 0" @input="onChangeColor"></v-select>
									</div>

									<label class="col-sm-2 control-label no-padding-right"> Size </label>
									<div class="col-sm-3">
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"> Pur. Rate </label>
									<div class="col-sm-4">
								...
								<div class="form-group">
									...
									<label class="col-sm-2 control-label no-padding-right"> S. Price </label>
									<div class="col-sm-3">
										<input type="text" id="sellingPrice" name="sellingPrice" class="form-control" v-model="selectedProduct.Product_SellingPrice"/>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right" for="productImage"> Image </label>
									<div class="col-sm-9">
										<input type="file" ref="productImage" id="productImage" name="productImage" class="form-control" @change="previewImage"/>
									</div>
								</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr>
							...
							<th style="width:14%;color:#000;">Category</th>
							<th style="width:15%;color:#000;">Product Name</th>
							<th style="width:5%;color:#000;">Size</th>
							<th style="width:5%;color:#000;">Color</th>
							<th style="width:14%;color:#000;">Purchase Rate</th>
							<th style="width:14%;color:#000;">Sale Rate</th>
							<th style="width:14%;color:#000;">Quantity</th>
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td><input type="number" class="form-control" v-model="product.purchaseRate" v-on:input="calculateCart(product)"></td>
							<td><input type="number" class="form-control" v-model="product.salesRate"></td>
							<td><input type="number" class="form-control" v-model="product.quantity" v-on:input="calculateCart(product)"></td>
							<td>{{ product.total }}</td>
						...
						<tr>
							...
							<td style="padding-top: 15px;font-size:18px;">{{ cart.reduce((prev, curr)=>{
								return prev + parseFloat(curr.quantity);
							},0) }}</td>
							<td style="padding-top: 15px;font-size:18px;">{{ purchase.subTotal }}</td>
							<td></td>
						</tr>
					</tbody>
				</table>
	...
	new Vue({
		...
		methods:{
			...
			onChangeCategory(){
				...
				if (this.purchase.purchaseId != 0 && this.lastProduct != null) {
					this.selectedColor = {
						color_SiNo : this.lastProduct.color,
						color_name : this.lastProduct.color_name
					};

					this.selectedSize = {
						size_SiNo : this.lastProduct.size,
						size_name: this.lastProduct.size_name,
					};

					this.selectedProduct = {
						Product_SlNo: '',
						Product_Name: this.lastProduct.Product_Name,
						quantity: this.lastProduct.PurchaseDetails_TotalQuantity,
						Product_Purchase_Rate: this.lastProduct.PurchaseDetails_Rate,
						Product_SellingPrice: this.lastProduct.Product_SellingPrice,
						total: this.lastProduct.PurchaseDetails_TotalAmount
					};
			...
			getProducts(){
				axios.post('/get_categories', {isService: 'false'}).then(res=>{
			...
			addToCart(){
				...
				if(this.selectedCategory == null || this.selectedCategory.ProductCategory_SlNo == null){
					alert('Select Category');
					return;
				}

				if(this.selectedProduct == null || this.selectedProduct.Product_Name == null){
				...
				if(this.selectedSize == null || this.selectedSize.size_SiNo == null){
					alert('Select Size');
					return;
				}
				if(this.selectedProduct.Product_SellingPrice == 0){
					alert('Selling Price Required');
					return;
				}

				let cartInd = this.cart.findIndex(p => {
					if(p.name == this.selectedProduct.Product_Name && p.colorId == this.selectedColor.color_SiNo && p.sizeId == this.selectedSize.size_SiNo){
						return true;
					}
					return false;
				});
			...
			calculateCart(item){
				item.total = (parseFloat(item.purchaseRate) * parseFloat(item.quantity)).toFixed(2);
				this.calculateTotal();
			},
			calculateTotal(){
			...
			savePurchase(){
				...
				formData.append('cartProducts', JSON.stringify(this.cart));
			...
			getPurchase(){
				axios.post('/get_purchases', {purchaseId: this.purchase.purchaseId}).then(res => {
					...
					r.purchaseDetails.forEach( (product, i) => {
						let cartProduct = {
						...
						if (i  === r.purchaseDetails.length - 1) {
