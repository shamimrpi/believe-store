    ...
    public function addSales(){
        ...
        try{
          ...
          $sales = array(
            ...
            'Status'                         => 'a',
            "AddBy"                          => $this->session->userdata("BRANCHid"),
        ...
        $salesId = $this->db->insert_id();
              ...
              foreach($carts as $key=>$cart){
                $saleDetails = [
                        ...
                        'SaleDetails_TotalQuantity' => $cart->quantity,
                        'Purchase_Rate' => $cart->purchase_price,
                        'SaleDetails_TotalAmount' =>$cart->selling_price,
                        'Status' => 'd',
                        'AddBy' => 'Admin',
                        'AddTime' => date('Y-m-d H:i:s'),
                        'SaleDetails_BranchId' => $this->sbrunch = $this->session->userdata('BRANCHid'),
                    ];
...
//    }
    ...
    public function orderHold(){
        ...
        $data['content'] = $this->load->view('Administrator/sales/order_hold', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getOrderHold(){
        ...
        sm.Status,
        sm.payment_type,
        sm.SaleMaster_Advance as advance,
        sm.SalseCustomer_IDNo,
        c.Customer_Code,
        c.Customer_Name,
        c.Customer_Mobile,
        ...
        join tbl_customer as c on c.Customer_SlNo = sm.SalseCustomer_IDNo
        where sm.Status = 'p'")->result();
    ...
    public function getOrderCancel(){
        ...
        from tbl_salesmaster as sm 
        join tbl_customer as c on c.Customer_SlNo = sm.SalseCustomer_IDNo
        where sm.Status = 'can'")->result();
        echo json_encode($data);
    }

    public function getOrderHoldUpdate($id){
        $updateData = array( 
            'Status'      => 'pro', 
        );
    ...
    public function getOrderDetails($id){
        ...
        $data['content'] = $this->load->view('Administrator/sales/orderDetails', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function orderComplete(){
        $data['title'] = 'Complete Order';
        $data['content'] = $this->load->view('Administrator/sales/order_complete', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function orderProcess(){
        $data['title'] = 'Process Order';
        $data['content'] = $this->load->view('Administrator/sales/order_process', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function orderCancel(){
        $data['title'] = 'Cancel Order';
        $data['content'] = $this->load->view('Administrator/sales/order_cancel', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
}
