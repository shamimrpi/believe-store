...
class Model_Table extends CI_Model{
    ...
    public function generateSalesInvoice2(){
        $branchId = $this->session->userdata('marchant_id');
        $branchNo = strlen($branchId) < 10 ? '0' . $branchId : $branchId;
        $invoice = date('y') . $branchNo . "00001";
        $year = date('y');
        $sales = $this->db->query("SELECT sm.SaleMaster_SlNo FROM tbl_salesmaster as sm ORDER BY AddTime DESC LIMIT 1")->row();
        if($sales !=NULL){
            $salse_id = $sales->SaleMaster_SlNo;
            if($salse_id != 0){
                $salse_id = $salse_id + 1;
                $zeros = array('0', '00', '000', '0000');
                $invoice = $year . $branchNo . (strlen($salse_id) > count($zeros) ? $salse_id : $zeros[count($zeros) - strlen($salse_id)] . $salse_id);
            }
    ...
    public function productStock($productId) {
        $stockQuery = $this->db->query("select * from tbl_currentinventory where product_id = ? ", [$productId]);
    ...
    public function marchantInvoice(){
        $branchId = $this->session->userdata('marchant_id');
        $branchNo = strlen($branchId) < 10 ? '0' . $branchId : $branchId;
        $invoice = date('y') . $branchNo . "00001";
        $year = date('y');
